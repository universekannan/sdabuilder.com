...
class GstInvoice1 extends MyPDF{
	//public $CI=null;
    ...
    public function _get_invoice_details()
    {
        ...
        $h = 100;
        $invoice_details = "";
        $invoice_details = '<span style="font-style:italic;">'.$this->CI->lang->line('invoice_details').'</span>';
	...
	{	
        ...
        if(!empty($store->postcode)){
		 ...
		 if($this->_invoice_format=='Default'){
               $gst =$store->vat_no; 
			 $label = $this->CI->lang->line("gst_number");
            }
            else{
               $gst = $store->gst_no;
				 $label = $this->CI->lang->line("gst_number");
		...
		if(!empty($customer->gstin)){
			$cuslabel = $this->CI->lang->line('gst_number');
			$cusgst = $customer->gstin;
		}
		if(!empty($customer->tax_number)){
			$cuslabel = $this->CI->lang->line('tax_number');
			$cusgst = $customer->tax_number;
		}
    ...
    <h3>'.strtoupper($this->_invoice_name).'</h1>
		 ...
		 <table class="table table-bordered">
                                    ...
                                    <tr>
										...
										<b><span style="font-size:16px;font-weight:500">'.$store->store_name.'</span><br>
										<span style="font-size:12px;">'.$store->address.'</span><br><span style="font-size:12px;">'.$address_line_2.'</span>
										<br>'.$this->CI->lang->line("contacts").': </b>'.$store->mobile.'<b><br>'.$label.': </b>'.$gst.'<br><b>'.$this->CI->lang->line("email").': </b>'.$store->email.'</td>
                                        <td colspan="2">'.$this->CI->lang->line("invoice_no").': '.$sales->sales_code.'</td>
                                        <td colspan="2">'.$this->CI->lang->line("date").' :<span style=""> '.show_date($sales->sales_date).'</span></td>
                                    </tr>
                                    <tr>
                                        <td colspan="2">'.$this->CI->lang->line("due_date").' :<span style=""> '.((!empty($sales->due_date)) ? show_date($sales->due_date):'').'</span></td>
                                        <td colspan="2">'.$this->CI->lang->line("payment_status").' :<span style=""> '.$sales->payment_status.'</span></td>
                                    </tr>
                                     <tr>
                                        <td colspan="2">'.$this->CI->lang->line("reference_no").' :<span style=""> '.$sales->reference_no.'</span></td>
                                        <td colspan="2">Other Reference(s)</td>
                                    </tr> 
                                    <tr>
                                        <td rowspan="5" colspan="4"><b>Buyer</b><br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'.strtoupper($customer->customer_name).'<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'.$customer->address.' '.$customer->postcode.'<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'.$customer->mobile.'<br>'.$customer->email.'
                                        <br>'.$cuslabel.'  '.$cusgst.'<br>
                                        </td>
		...
		$tbl .='<table nobr="true">
		            <tbody>
		                <tr nobr="true">
		                    ...
		                    <td colspan="6" style="vertical-align:bottom;min-height:60px;height:60px;">
		                    	<div style="font-size:10px;">
								<span style="color:rgb(65, 59, 212);font-style:italic;">'.$this->CI->lang->line("bank_details").'</span><br>
'.nl2br($store->bank_details).'<br>

<!--<b style="display:block;text-align: center;">for SDA POWER TOOLS </b>--><br><br><br>
