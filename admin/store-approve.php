...
if (isset($_POST['update'])) {
	...
	$date =date('y/m/d'); 
	    ...
	    $stmt = $conn->prepare("Update supervisor set status=?,date=? where id=?");
        $stmt->bind_param("sss",$status,$date,$id);
        $stmt->execute() or die($stmt->error);
	
	$stmt = $conn->prepare("Update staff set status=?,date=? where id=?");
        $stmt->bind_param("sss",$status1,$date,$id);
        $stmt->execute() or die($stmt->error);
                            ...
                            <div class="row">
                               ...
                               <form method="post" action="" enctype="multipart/form-data">
                                            ...
                                            <select disabled  name="project_id" class="form-control" required="required" >
                                                <?php
                                                $sql2 = "select * from project  where project_type='Store' order by id";
                                ...
                                <div class="col-md-12 text-center">
                                    <input required="required" class="btn btn-info"
                                           type="submit"
                                           name="update" value="Approve"/>
                                    <a href="store-pending.php" class="btn btn-info">Back</a>
                                </div>
                                        </form>
